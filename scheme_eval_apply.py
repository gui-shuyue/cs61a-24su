...
def eval_all(expressions, env):
    ...
    while expressions.rest is not nil:
        scheme_eval(expressions.first, env, False)  # 不是尾调用
        expressions = expressions.rest
    
    # 最后一个表达式在尾位置
    return scheme_eval(expressions.first, env, True)  # 是尾调用
    """
...
#         child_frame = procedure.env.make_child_frame(procedure.formals, args)
#         return eval_all(procedure.body, child_frame)  # eval_all已经处理了尾调用
#     elif isinstance(procedure, MuProcedure):
#         child_frame = env.make_child_frame(procedure.formals, args)
#         return eval_all(procedure.body, child_frame)  # eval_all已经处理了尾调用
        ...
        while isinstance(result, Unevaluated):
            ...
            if scheme_symbolp(expr):
                ...
                if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
                    ...
                    args = rest.map(lambda operand: optimized_eval(operand, env))

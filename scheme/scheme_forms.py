...
# def do_begin_form(expressions, env):
#     validate_form(expressions, 1)
#     return eval_all(expressions, env)  # eval_all已经处理了尾调用
...
def do_if_form(expressions, env):
    validate_form(expressions, 2, 3)
    if is_scheme_true(scheme_eval(expressions.first, env, False)):  # 条件不在尾位置
        return scheme_eval(expressions.rest.first, env, True)  # then分支在尾位置
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True)  # else分支在尾位置
"""
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        value = scheme_eval(expressions.first, env, False)  # 非最后一个不在尾位置
        if is_scheme_false(value):
            return value
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, True)  # 最后一个在尾位置
"""
